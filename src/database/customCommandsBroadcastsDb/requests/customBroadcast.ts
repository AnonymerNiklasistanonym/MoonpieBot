...
export const existsEntry = async (
  ...
  try {
    const runResultExists = await db.requests.getEach<ExistsDbOut>(
      ...
      db.queries.exists(customBroadcastsTable.name, {
...
export const createEntry = async (
  ...
  const postResult = await db.requests.post(
    databasePath,
    db.queries.insert(customBroadcastsTable.name, columns),
    values,
    logMethod
  );
...
export const removeEntry = async (
  ...
  const postResult = await db.requests.post(
    databasePath,
    db.queries.remove(customBroadcastsTable.name, {
      columnName: customBroadcastsTable.columns.id.name,
    }),
    [input.id],
    logMethod
...
export const updateEntry = async (
  ...
  const postResult = await db.requests.post(
    databasePath,
    db.queries.update(customBroadcastsTable.name, columns, {
      columnName: customBroadcastsTable.columns.id.name,
    }),
    [...values, input.id],
    logMethod
