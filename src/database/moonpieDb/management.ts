...
export const setup = async (
  ...
  const logMethod = createLogMethod(logger, "database_moonpie_setup");
  ...
  await db.requests.post(
    ...
    db.queries.createTable(
      moonpieTable.name,
      Object.values(moonpieTable.columns),
      true
    ),
    undefined,
    logMethod
  );
  // > Create moonpie leaderboard view if not existing
  await db.requests.post(
    databasePath,
    db.queries.createView(
      moonpieLeaderboardView.name,
      moonpieLeaderboardView.tableName,
      Object.values(moonpieLeaderboardView.columns),
      moonpieLeaderboardView.options,
      true
    ),
    undefined,
    logMethod
  ...
  loggerDatabase.debug(
    `Database tables were created or found existing '${databasePath}'`
  );
