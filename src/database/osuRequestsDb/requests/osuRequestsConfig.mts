...
export const existsEntry = async (
  ...
  try {
    const runResultExists = await db.default.requests.getEach<ExistsDbOut>(
      ...
      db.default.queries.exists(osuRequestsConfigTable.name, {
...
export const createEntry = async (
  ...
  const postResult = await db.default.requests.post(
    databasePath,
    db.default.queries.insert(osuRequestsConfigTable.name, [
...
export const removeEntry = async (
  ...
  const postResult = await db.default.requests.post(
    databasePath,
    db.default.queries.remove(osuRequestsConfigTable.name, {
      columnName: osuRequestsConfigTable.columns.option.name,
    }),
    [input.option],
    logMethod
...
export const updateEntry = async (
  ...
  const postResult = await db.default.requests.post(
    databasePath,
    db.default.queries.update(osuRequestsConfigTable.name, columns, {
      columnName: osuRequestsConfigTable.columns.option.name,
    }),
    [...values, input.option],
    logMethod
