...
export const existsEntry = async (
  ...
  try {
    const runResultExists = await db.default.requests.getEach<ExistsDbOut>(
      ...
      db.default.queries.exists(spotifyConfigTable.name, {
...
export const createEntry = async (
  ...
  const postResult = await db.default.requests.post(
    databasePath,
    db.default.queries.insert(spotifyConfigTable.name, [
...
export const removeEntry = async (
  ...
  const postResult = await db.default.requests.post(
    databasePath,
    db.default.queries.remove(spotifyConfigTable.name, {
      columnName: spotifyConfigTable.columns.option.name,
    }),
    [input.option],
    logMethod
...
export const getEntries = async (
  ...
  const runResult = await db.default.requests.getAll<GetOsuRequestsConfigDbOut>(
    databasePath,
    db.default.queries.select(spotifyConfigTable.name, [
      {
        alias: "option",
        columnName: spotifyConfigTable.columns.option.name,
      },
...
export const updateEntry = async (
  ...
  const postResult = await db.default.requests.post(
    databasePath,
    db.default.queries.update(spotifyConfigTable.name, columns, {
      columnName: spotifyConfigTable.columns.option.name,
    }),
    [...values, input.option],
    logMethod
