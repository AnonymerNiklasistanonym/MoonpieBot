  ...
  switch (commentInstruction.instruction) {
    case CommentInstructionType.DEFAULT_CONFIG_DIR_LOCATION:
      output += `The default location of the configuration/database/etc. files is \`${defaultConfigDir(
        commentInstruction.argument === "WINDOWS"
      )}\`.\n`;
      break;
    case CommentInstructionType.DEFAULT_CONFIG_DIR_SECTION:
      output += "- set environment variables\n";
      output += `- list the environment variables in a \`${fileNameEnv}\` file in the same directory you run \`${binaryName}\`\n`;
      ...
      output += `\nNote: Via the command line argument \`${CliOption.CONFIG_DIRECTORY}\` or the Windows installer shortcut for a custom configuration directory you can always change the directory which is searched for the \`${fileNameEnv}\` and other configuration files.\n`;
      break;
...
const updateReadmeFile = async (mdFilePath: string) => {
  ...
  for (const line of readmeFileLines.toString().split("\n")) {
    ...
    if (lookForEnd === false && mdComment.startsWith(mdCommentText.begin)) {
...
const createEnvExample = async (
  ...
  const moonpieConfig = await getMoonpieConfigFromEnv(path.join(__dirname));
