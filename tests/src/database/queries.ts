...
export default (): Mocha.Suite => {
  return describe("queries", () => {
    it("create table", () => {
      ...
      chai
        .expect(queryCreateTable1)
        .to.deep.equal(
          "CREATE TABLE test " +
            "(blob BLOB,integer INTEGER,numeric NUMERIC,real REAL,text TEXT);"
        );

      const queryCreateTable2 = database.queries.createTable(
        "test",
        columns,
        true
      );
      ...
      chai
        .expect(queryCreateTable2)
        .to.deep.equal(
          "CREATE TABLE IF NOT EXISTS test " +
            "(blob BLOB,integer INTEGER,numeric NUMERIC,real REAL,text TEXT);"
        );

      const queryCreateTable3 = database.queries.createTable(
        "contact_groups",
        [
          {
            foreign: {
    ...
    it("insert", () => {
      const queryInsert = database.queries.insert("test", [
        "column1",
        "column2",
      ]);
      ...
      chai
        .expect(queryInsert)
        .to.deep.equal("INSERT INTO test(column1,column2) VALUES(?,?);");
    });
    it("remove", () => {
      ...
      chai
        .expect(queryRemove2)
        .to.deep.equal("DELETE FROM test WHERE columnWhere=?;");
    });
    it("select", () => {
      ...
      const querySelect2 = database.queries.select("test", ["a", "b", "c"], {
        whereColumn: "id",
      });
      ...
      chai
        .expect(querySelect2)
        .to.deep.equal("SELECT a,b,c FROM test WHERE id=?;");
      ...
      chai
        .expect(querySelect3)
        .to.deep.equal("SELECT a,b,c FROM test WHERE test.id=?;");
    ...
    it("update", () => {
      ...
      chai
        .expect(queryUpdate1)
        .to.deep.equal("UPDATE test SET a=?,b=?,c=? WHERE id=?;");

      const queryUpdate2 = database.queries.update(
        "test",
        ["a", "b", "c"],
        "whereColumn"
      );
      ...
      chai
        .expect(queryUpdate2)
        .to.deep.equal("UPDATE test SET a=?,b=?,c=? WHERE whereColumn=?;");
    });
  });
};
